...
function queryBureauForUpdate() {
    $.ajax({
        ...
        success: function (data) {
            ...
            queryUpdateLocationCode(data.locationCode,data.smallCategoryName);
...
//根据所在行政区域码让该单位行政区域显示出来
function queryUpdateLocationCode(locationCode,locationName) {
	$.ajax({
		type:"POST",
		dataType:"JSON",
		url:"bureau/queryUpdateLocationCode",
		data:{
		...
		success:function(data) {
			console.log(data);
			if(data.locationProvinceCode != null){
				getSelectData($($(".dropdown-toggle")[0]),"Province");
				$("#province").selectpicker('val',data.locationProvinceCode);
			}if(data.locationCityCode != null){
				getSelectData($($(".dropdown-toggle")[1]),data.locationProvinceCode);
				$("#city").selectpicker('val',data.locationCityCode);
			}if(data.locationAreaCode != null){
				getSelectData($($(".dropdown-toggle")[2]),locationCityCode);
				$("#region").selectpicker('val',data.locationAreaCode);
			}
			
		},
		error:function(data) {
			layer.msg("显示行政区域失败！", {icon : 2});
		}
	});
}
